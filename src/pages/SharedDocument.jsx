...
import { Eye, FileText, KeyRound, Loader2, Lock, ShieldCheck } from "lucide-react";
...
function ProtectedViewer({ documentInfo, viewUrl }) {
  ...
  return (
    <div className="overflow-hidden rounded-[2rem] bg-slate-950 p-2 ring-1 ring-[#EFE4D6]">
      <div
        className="relative flex h-[72vh] min-h-[420px] items-center justify-center overflow-hidden rounded-[1.5rem] bg-white"
        onContextMenu={(event) => event.preventDefault()}
      >
        {isImage && (
          <img
            src={viewUrl}
            alt={documentInfo.documentName || documentInfo.fileName || "Document Camelio"}
            draggable={false}
        ...
        {isPdf && (
          <iframe
            src={pdfUrl}
            title={documentInfo.documentName || documentInfo.fileName || "Document Camelio"}
        ...
        {!isImage && !isPdf && (
          <div className="max-w-md px-6 text-center">
            ...
            <p className="mt-2 text-sm leading-6 text-[#746F64]">
              Le mode visionnement seulement est activé. L’aperçu intégré fonctionne mieux avec les PDF et les images.
            </p>
          </div>
        )}
...
export default function SharedDocument() {
  ...
  async function requestAccess() {
    ...
    try {
      ...
      if (!url) {
        throw new Error("Le serveur n’a pas retourné de lien de visionnement.");
      }

      setDocumentInfo((current) => ({
        ...(current || {}),
        documentName: data?.documentName || current?.documentName,
              ...
              <div className="rounded-3xl bg-[#F4F8FD] p-4 ring-1 ring-[#D3DFF1]">
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="text-xs font-bold uppercase tracking-wide text-[#8A8175]">Document partagé</p>
              ...
              {!viewUrl && (
                <>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-sm leading-6 text-[#746F64]">
                          Le document sera affiché dans Camelio. Le téléchargement est retiré de cette interface.
                        </p>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="flex gap-2">
                      <div className="relative flex-1">
                        ...
                        <input
                          ...
                          onKeyDown={(event) => {
                            if (event.key === "Enter") requestAccess();
                          }}
                          placeholder="A7K2"
                  ...
                  >
                    ...
                    {accessing ? "Vérification..." : "Visionner le document"}
                  </button>
                </>
              )}

              {viewUrl && <ProtectedViewer documentInfo={documentInfo} viewUrl={viewUrl} />}
            </div>
          )}
        </div>

        <p className="mt-5 text-center text-xs leading-5 text-[#8A8175]">
          Note : le mode visionnement seulement retire le téléchargement de l’interface partagée, mais une personne peut toujours faire une capture d’écran.
        </p>
      </div>
    </div>
  );
